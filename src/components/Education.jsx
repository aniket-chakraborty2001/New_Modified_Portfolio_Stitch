...
function EducationTrace({ item, index }) {
  ...
  return (
    <article
      className={`education-trace-row group relative grid min-h-[16rem] gap-5 border-y border-[#20b2aa]/45 py-5 sm:py-7 lg:grid-cols-[minmax(0,1fr)_5.5rem_minmax(0,1fr)] lg:items-center lg:gap-8 ${
        isReversed ? "lg:[&_.education-copy]:col-start-3" : ""
      }`}
    >
      <span
        aria-hidden="true"
        className="education-row-scan pointer-events-none absolute inset-x-0 top-0 h-px origin-left bg-gradient-to-r from-[#0ea5e9] via-[#20b2aa] to-[#d946ef]"
      />
      ...
      >
        ...
        <h3 className="education-title mt-4 break-words text-xl font-black leading-tight tracking-[0] text-[#0ea5e9] sm:text-2xl lg:text-3xl">
          {item.institution}
        </h3>
        <p className="education-degree mt-2 break-words text-base font-black leading-tight text-[#172033] drop-shadow-[0_3px_0_rgba(14,165,233,0.12)] sm:text-lg lg:text-xl">
          {item.degree}
        </p>
      ...
      <div
        aria-hidden="true"
        className={`education-connector hidden h-px bg-gradient-to-r from-transparent via-[#20b2aa]/80 to-transparent lg:row-start-1 lg:block ${
...
export default function Education() {
  ...
  return (
    <section
      id="education"
      ref={sectionRef}
      className="relative z-10 mx-auto h-[260svh] w-full max-w-[92rem] scroll-mt-20 px-5 sm:px-8 lg:px-12"
    >
      ...
      <div className="sticky top-0 flex min-h-screen flex-col justify-center overflow-hidden py-20">
        ...
        <div className="relative mx-auto mt-8 w-full max-w-6xl sm:mt-10 lg:mt-12">
          ...
          >
            <span className="education-rail-fill block h-full w-full bg-gradient-to-b from-[#0ea5e9]/45 via-[#20b2aa]/85 to-[#d946ef]/45" />
          </div>
